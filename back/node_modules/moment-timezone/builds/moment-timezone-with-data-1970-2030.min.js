! function(c, M) {
    ...
    function p(c) {
        var M = 0,
            ...
            b = 0,
            a = 1;
        for (45 === c.charCodeAt(0) && (a = -(M = 1)); M < A.length; M++) b = 60 * b + n(A.charCodeAt(M));
        for (M = 0; M < o.length; M++) i /= 60, b += n(o.charCodeAt(M)) * i;
        return b * a
    }

    function q(c) {
        for (var M = 0; M < c.length; M++) c[M] = p(c[M])
    }
    ...
    function d(c) {
        for (var c = c.split("|"), M = c[2].split(" "), a = c[3].split(""), A = c[4].split(" "), o = (q(M), q(a), q(A), A), i = a.length, b = 0; b < i; b++) o[b] = Math.round((o[b - 1] || 0) + 6e4 * o[b]);
        return o[i - 1] = 1 / 0, {
            name: c[0],
            abbrs: O(c[1].split(" "), a),
            offsets: O(M, a),
    ...
    function L() {
        for (var c, M, a, A = (new Date).getFullYear() - 2, o = new N(new Date(A, 0, 1)), i = o.offset, b = [o], n = 1; n < 48; n++)(a = new Date(A, n, 1).getTimezoneOffset()) !== i && (c = function(c, M) {
            for (var a; a = 6e4 * ((M.at - c.at) / 12e4 | 0);)(a = new N(new Date(c.at + a))).offset === c.offset ? c = a : M = a;
            return c
        }(o, M = new N(new Date(A, n, 1))), b.push(c), b.push(new N(new Date(c.at + 6e4))), o = M, i = a);
        for (n = 0; n < 4; n++) b.push(new N(new Date(A + n, 0, 1))), b.push(new N(new Date(A + n, 6, 1)));
        return b
    }
        ...
        for (var a, A, o = L(), i = o.length, b = function(c) {
                for (var M, a, A, o = c.length, i = {}, b = [], n = {}, p = 0; p < o; p++)
                    if (a = c[p].offset, !n.hasOwnProperty(a)) {
                        for (M in A = r[a] || {}) A.hasOwnProperty(M) && (i[M] = !0);
                        n[a] = !0
                    } for (p in i) i.hasOwnProperty(p) && b.push(e[p]);
                return b
            }(o), n = [], p = 0; p < b.length; p++) {
            for (a = new u(X(b[p])), A = 0; A < i; A++) a.scoreOffsetAt(o[A]);
            n.push(a)
        }
    ...
    function s(c) {
        ...
        for ("string" == typeof c && (c = [c]), M = 0; M < c.length; M++) {
            ...
            var i, b, n = o,
                p = A[2].split(" ");
            for (q(p), i = 0; i < p.length; i++) b = p[i], r[b] = r[b] || {}, r[b][n] = !0
        }
    }
    ...
    function m(c) {
        var M, a, A, o;
        for ("string" == typeof c && (c = [c]), M = 0; M < c.length; M++) A = B((a = c[M].split("|"))[0]), o = B(a[1]), i[A] = o, e[A] = a[0], i[o] = A, e[o] = a[1]
    }

    function T(c) {
        s(c.zones), m(c.links);
        var M, a, A, o = c.countries;
        if (o && o.length)
            for (M = 0; M < o.length; M++) a = (A = o[M].split("|"))[0].toUpperCase(), A = A[1].split(" "), b[a] = new t(a, A);
    ...
    function C(c) {
        ...
        _index: function(c) {
            c = function(c, M) {
                ...
                if (c >= M[A - 1]) return -1;
                for (var o = 0, i = A - 1; 1 < i - o;) M[a = Math.floor((o + i) / 2)] <= c ? o = a : i = a;
                return i
            }(+c, this.untils);
            if (0 <= c) return c
        },
        ...
        var c, M = [];
        for (c in e) e.hasOwnProperty(c) && (z[c] || z[i[c]]) && e[c] && M.push(e[c]);
        return M.sort()
